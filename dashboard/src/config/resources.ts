...
export const resourceConfigs: ResourceConfig[] = [
  {
    ...
    columns: [
      ...
      makeColumn('is_active', 'Active', 90),
    ...
    formFields: [
      ...
      makeField('is_active', 'Active', 'boolean'),
    ],
  ...
  },
  {
    ...
    columns: [
      ...
      makeColumn('is_active', 'Active', 120),
  ...
  },
  {
    id: 'user_roles',
    label: 'User Roles',
    translationKey: 'resources.tables.userRoles',
    endpoint: 'user_roles',
    primaryKey: 'user_code',
    allowAdd: true,
    allowUpdate: false,
    allowDelete: true,
    rowId: (row) => `${row.user_code}-${row.role_id}`,
    columns: [
      makeColumn('user_code', 'User code', 180),
      makeColumn('role_id', 'Role ID', 120),
    ],
    formFields: [
      makeField('user_code', 'User code', 'text', true),
      makeField('role_id', 'Role ID', 'number', true),
    ],
    searchFields: [makeSearch('user_code', 'User code'), makeSearch('role_id', 'Role ID', 'number')],
    buildDeletePath: (row) => `/user_roles/${row.user_code}/${row.role_id}`,
  },
  {
    id: 'terms',
  ...
  },
  {
    ...
    formFields: [
      makeField('plan_code', 'Code', 'text', true),
    ...
    searchFields: [makeSearch('plan_code', 'Code'), makeSearch('plan_name', 'Name')],
  },
  {
    id: 'subscriptions',
