...
export default function Navbar() {
  ...
  const unreadCount = React.useMemo(() => notifications.filter((item) => !item.is_read).length, [notifications]);

  const notificationScrollDebounceRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  const handleNotificationScroll = (event: React.UIEvent<HTMLDivElement>) => {
    ...
    notificationScrollDebounceRef.current = setTimeout(() => {
      ...
      if (nearBottom && notificationsQuery.hasNextPage && !notificationsQuery.isFetchingNextPage) {
        notificationsQuery.fetchNextPage().catch((err) => console.error('Failed to load more notifications', err));
      }
    }, 120);
  };
  ...
  <>
    ...
    <Box className="top-navbar">
      <Box className="ps-container top-navbar-inner">
        <Stack direction="row" alignItems="center" justifyContent="space-between" className="top-navbar-row">
          <Stack direction="row" alignItems="center" spacing={2} className="top-navbar-left">
            <Stack direction="row" alignItems="center" spacing={0.75} className="top-navbar-phone">
            ...
            <Stack direction="row" alignItems="center" spacing={1} className="top-navbar-social">
              <IconButton
      ...
      <Box className="ps-container navbar-inner">
        <Stack className="navbar-content" direction="row" alignItems="center" justifyContent="space-between">
            ...
            <Box className="navbar-links">
              {navLinks.map((link) => (
                <MuiLink
                  ...
                  sx={{ pointerEvents: isLanguageChanging ? 'none' : 'auto', color: '#000000' }}
                >
          ...
          <Stack direction="row" spacing={2} alignItems="center" className="nav-actions">
            <Box ref={notificationDropdown.containerRef} className="dropdown-container">
              <IconButton
              ...
              {notificationDropdown.open && (
                <Box className="dropdown-panel dropdown-panel--notifications">
                  <Box className="notification-header">
                    ...
                    <Typography variant="caption" className="notification-subtitle">
                      {t('nav.unread', { defaultValue: '{{count}} mới', count: unreadCount })}
                    </Typography>
                  </Box>

                  <Box className="notification-list" onScroll={handleNotificationScroll}>
                    {notificationsQuery.isLoading ? (
                      <Typography variant="body2" className="notification-empty">
                        {t('common.loading', { defaultValue: 'Loading...' })}
                      </Typography>
                    ) : notifications.length === 0 ? (
                      <Typography variant="body2" className="notification-empty">
                        {t('nav.noNotifications', { defaultValue: 'Chưa có thông báo.' })}
                      </Typography>
                    ) : (
                      notifications.map((item) => (
                        <Box key={item.id} className={`notification-item ${item.is_read ? '' : 'is-unread'}`}>
                          <Box className="notification-item-top">
                            <Typography variant="body2" className="notification-item-title">
                              {item.title}
                            </Typography>
                            <Typography variant="caption" className="notification-item-time">
                              {new Date(item.created_at).toLocaleString()}
                            </Typography>
                          </Box>
                          <Typography variant="body2" className="notification-item-desc">
                            {item.content}
                          </Typography>
                        </Box>
                      ))
                    ...
                    {notificationsQuery.isFetchingNextPage && (
                      <Typography variant="caption" className="notification-empty">
                        {t('common.loading', { defaultValue: 'Loading...' })}
                      </Typography>
                    )}
                  </Box>
